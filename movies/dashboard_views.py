...
def movie_list(request):
    ...
    theater_name = request.GET.get('theater', '').strip()

    release_date = request.GET.get(
        'release_date',
        ''
    ).strip()
    ...
    movies = (
        ...
        .annotate(
            ...
            minimum_ticket_price=Min(
                'theater__seat__price'
            )
        )
    )
    ...
    # ------------------------------------------------------
    #
    # This assumes Theater has a "city" field.
    # If your Theater model uses another field such as
    # location/city_name, change ONLY this lookup.
    #

    if city:
        movies = movies.filter(
            theater__location__icontains=city
        )
    ...
    if theater_name:
        movies = movies.filter(
            theater__name__icontains=theater_name
        )
    ...
    if show_time:
        movies = movies.filter(
            theater__time__icontains=show_time
        )
    ...
    matching_movie_count = movies.count()
    ...
    # ------------------------------------------------------
    #
    # Theater uses the location field for city/location filtering.
    #
    ...
    recently_viewed_movies = [
        recently_viewed_map[movie_id]
        for movie_id in recently_viewed_ids
        if movie_id in recently_viewed_map
    ][:6]
    ...
    if request.user.is_authenticated:
        ...
        # Movies previously booked by the user
        ...
        # Get genres/languages from booking history
        booked_preferences = Movie.objects.filter(
        ...
        for item in booked_preferences:
...
def payment_page(request, theater_id):
    ...
    try:
        ...
        payment.stripe_order_id_order_id = (
            checkout_session.id
        )
